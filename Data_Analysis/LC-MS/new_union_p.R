...
new.null <- function(stat,name){
  if (name=="p"){
  ZmatW_b = whole(ZZ="Z",stat=stat,cancer="pb",name=name)
  ZtildematW_b= whole(ZZ="Ztilde",stat=stat,cancer="pb",name=name)
  ZmatW_c = whole(ZZ="Z",stat=stat,cancer="pc",name=name)
  ZtildematW_c= whole(ZZ="Ztilde",stat=stat,cancer="pc",name=name)
  }
  
  if (name=="aq"){
    ZmatW_b = whole(ZZ="Z",stat=stat,cancer="aqb",name=name)
    ZtildematW_b= whole(ZZ="Ztilde",stat=stat,cancer="aqb",name=name)
    ZmatW_c = whole(ZZ="Z",stat=stat,cancer="aqc",name=name)
    ZtildematW_c= whole(ZZ="Ztilde",stat=stat,cancer="aqc",name=name)
  }
  
  p=dim(ZmatW_b)[2]
  rep=dim(ZmatW_b)[1]
  
  ZmatW=ZtildematW=ZmatW_b
  for (i in 1:rep){
    ZmatW[i,]=ZmatW_b[i,]*ZmatW_c[i,]+ZtildematW_b[i,]*ZtildematW_c[i,]
    ZtildematW[i,]=ZmatW_b[i,]*ZtildematW_c[i,]+ZtildematW_b[i,]*ZmatW_c[i,]
  }
...
Lasso = new.null(stat="Lasso",name="p")$sumselect1[,2]
Lasso_Order = new.null(stat="Lasso_Order",name="p")$sumselect1[,2]
GMUL = new.null(stat="GMUL",name="p")$sumselect1[,2]
GDS = new.null(stat="GDS",name="p")$sumselect1[,2]
CocoLasso2 = new.null(stat="CocoLasso2",name="p")$sumselect1[,2]
RF = new.null(stat="RF",name="p")$sumselect1[,2]
...
write.csv(result_breast_p,"result_union_p.csv")

Lasso = new.null(stat="Lasso",name="aq")$sumselect1[,2]
Lasso_Order = new.null(stat="Lasso_Order",name="aq")$sumselect1[,2]
GMUL = new.null(stat="GMUL",name="aq")$sumselect1[,2]
GDS = new.null(stat="GDS",name="aq")$sumselect1[,2]
CocoLasso2 = new.null(stat="CocoLasso2",name="aq")$sumselect1[,2]
RF = new.null(stat="RF",name="aq")$sumselect1[,2]
...
write.csv(result_breast_aq,"result_union_aq.csv")
